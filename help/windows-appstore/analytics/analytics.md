...
1. 클릭 **[!UICONTROL 최신 앱 보고서 활성화]**.
   ...
   원할 경우 **[!UICONTROL Mobile 위치 추적 활성화]** 및 **[!UICONTROL 배경 히트에 대한 이전 보고 및 속성 활성화]**.
...
만약 [ADBMobile 클래스 및 메서드 참조](/help/windows-appstore/c-configuration/methods.md), events, eVar, prop, 상속자 및 목록을 어디에 설정해야 하는지 알 수 있습니다. 버전 4에서는 더 이상 이러한 유형의 변수를 앱에서 바로 할당할 수 없습니다. 대신, SDK는 컨텍스트 데이터 및 처리 규칙을 사용하여 앱 데이터를 Analytics 변수에 매핑해서 보고에 사용합니다.
...
설정하려면 *`products`* mobile SDK에서 특수 구문을 사용해야 합니다. 자세한 내용은 [Products 변수](/help/windows-appstore/analytics/products/products.md).

## (선택 사항) 오프라인 추적을 활성화합니다 {#section_955B2A03EB854742BDFC4A0A3C287009}

장치가 오프라인 상태일 때 히트를 저장하려면 [ADBMobileConfig.json 구성](/help/windows-appstore/c-configuration/methods.md). 오프라인 추적을 활성화하려면 먼저 구성 파일 참조에 설명된 타임스탬프 요구 사항에 유의하십시오.
...
* 위도/경도 및 POI(POI 내에 `ADBMobileConfig.json` 구성 파일)을 참조하십시오. 이러한 변수는 자동 보고를 위해 모바일 솔루션 변수로 전달됩니다.
...
장치 위치가 정의된 지점의 7000m 반경 내에 있는 것으로 확인되면 `a.loc.poi` 값이 &quot;San Francisco&quot;인 컨텍스트 데이터 변수가 `TrackLocation` 히트. An `a.loc.dist` 컨텍스트 변수가 정의된 좌표로부터 떨어진 거리(단위: 미터)와 함께 전송됩니다.
