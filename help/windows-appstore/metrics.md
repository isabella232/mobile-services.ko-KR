...
구성된 경우 라이프사이클 지표가 컨텍스트 데이터 매개 변수로 Analytics로, 매개 변수로 각 mbox 호출이 있는 Target으로, Audience Manager에 대한 신호로 전송됩니다. 분석 및 Target은 동일한 형식을 사용하고 Audience Manager은 각 지표에 대해 다른 접두사를 사용합니다.
...
다음 지표 및 차원은 설명에 나열된 방법으로 모바일 솔루션 변수에서 캡처됩니다.
