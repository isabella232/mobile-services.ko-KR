...
시작은 새 세션에서만 실행됩니다. 세션이 시작되는 시기에 대한 자세한 내용은 `lifecycleTimeout` ADBMobile JSON 구성 [파일](/help/ios/configuration/json-config/json-config.md) 을 참조하십시오.

## 내 메시지를 원격으로 업데이트했지만 아직 이전 메시지가 표시됩니다.

다음 작업 중 하나를 완료하십시오.

* 다이내믹 태그 관리는 새 정의로 종점을 업데이트하는 데 몇 분 정도 걸릴 수 있습니다.

   잠시 시간을 내어 다시 시도하십시오.

* 구성은 새로운 시작 시에만 업데이트됩니다.

   앱이 라이프사이클 세션 제한 시간에 다시 시작되었으면 새 구성이 다운로드되지 않았을 수 있습니다.
...
장치 화면의 크기가 다르기 때문에 이미지가 템플릿에서 제공한 공간에 정확히 맞지는 않을 수도 있습니다. 템플릿은 항상 이미지의 중앙을 표시하는 데 중점을 두며 이미지가 맞지 않을 경우 측면을 자르거나(세로) 측면에 페이드를 적용(가로)합니다.

다음은 각 방향에 대한 정확한 배치 및 크기 조정 규칙입니다.

* **세로**- 이미지의 높이가 휴대폰의 경우 195px, 태블릿의 경우 529px, 이미지 너비가 장치 너비보다 작은 경우 가운데에 표시되고, 이미지 너비가 장치 너비보다 큰 경우 잘립니다.

* **가로**- 이미지의 높이가 장치의 100%로 조정되고 너비는 장치의 75%이고 오른쪽에 페이드 아웃이 적용됩니다.

   전체 화면 템플릿에 문제가 있으면 사용자 지정 HTML 템플릿을 다운로드하여 사용할 수 있습니다. 사용자 지정 HTML 템플릿은 이미지에 대한 유연성을 높여주고 템플릿을 완전히 제어할 수 있습니다.

## 내 메시지는 UI에서 수행한 변경/업데이트를 반영하지 않습니다.

SDK는 라이프사이클 시작 시 새/업데이트된 메시지를 가져옵니다. 라이프사이클 시간 제한 값보다 큰 응용 프로그램이 닫힘/백그라운드에서 열린 경우에만 해당됩니다.
