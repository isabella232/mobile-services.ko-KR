...
보고서를 사용자 정의하기 전에 지표와 차원 간의 차이점을 이해해야 합니다.
...
예를 들어, 시작 스파이크가 표시되면, 상위 5개 운영 체제에 대한 시작을 보여주는 트렌드 보고서를 실행하여 어느 운영 체제가 시작 스파이크에 가장 많이 기여하고 있는지 확인할 수 있습니다.

![](assets/overtime.png)

기간 보고서에서 다른 지표와 함께 차원 값을 보려면, 인스턴스 지표를 사용하고 차원 필터를 정의하면 됩니다.
...
보려는 특정 차원 값이 있는 경우 기간 보고서에 고정 필터를 추가할 수 있습니다. 다음 보고서는 특정 운영 체제 버전에 대한 30일 동안의 시작, 업그레이드 및 충돌을 보여줍니다.
...
선버스트 보고서에서는 예를 들어 분류와 함께 기본 보고서를 제공합니다. 시각화는 높이를 사용하여 지표와 지표 간의 성능 차이를 표시합니다. 각 동심원은 해당 서클의 카테고리에서 대상 세그먼트를 나타냅니다. 고정 필터 적용, 지표 숨기기, 지표 보기 등 대상에 대한 작업을 수행할 수 있습니다.

선버스트 차트와 상호 작용하는 방법을 설명하는 제품 내 자습서 및 보고서를 볼 수 있습니다.
...
경로 지정 보고서는 경로 분석을 기반으로 하며 앱의 한 상태에서 다른 상태로 이동되는 경로를 나타내는 경로 지정 차트를 표시합니다.

![](assets/action_paths.png)

각 노드는 상자처럼 형성되며 앱을 통해 사용자 경로에서 상태를 나타냅니다. 예를 들어 위의 그림에서 맨 위 노드는 앱을 시작하고 갤러리에서 사진을 선택한 사용자의 수를 나타냅니다.
...
단계 시각화를 사용하면 고객이 프로세스에서 이탈한 위치를 확인할 수 있습니다. 각 단계에서 고객 의사 결정을 명확하게 파악하면 고객이 단념하는 위치, 고객이 따르는 경향, 앱이 이탈하는 시점을 이해할 수 있습니다.
