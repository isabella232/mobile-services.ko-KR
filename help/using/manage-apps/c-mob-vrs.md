...
* 데이터 액세스 제한
   ...
   다국적 기업에는 모든 지리적 위치에 대한 보고서 세트로 데이터를 전송하는 앱이 있습니다. 하지만 이 회사에서 한 지역의 비즈니스 사용자를 다른 지역의 데이터 보기로 제한하려고 합니다. 회사의 관리자는 지역별로 사용자를 세그먼트할 VRS를 만들고, 해당 지역을 관리하는 비즈니스 사용자에게만 이 VRS에 대한 권한을 제공할 수 있습니다.
...
>[!IMPORTANT]
>
>Adobe Analytics 관리자만 Adobe Analytics에서 가상 보고서 세트를 만들고 수정할 수 있습니다. 가상 보고서 세트를 만들려면 다음을 참조하십시오 [가상 보고서 세트 만들기](https://experienceleague.adobe.com/docs/analytics/components/virtual-report-suites/vrs-workflow/vrs-create.html?lang=ko-KR) Adobe Analytics 설명서에서 확인할 수 있습니다.
...
Adobe Mobile Services UI에서 VRS를 사용하여 앱을 만들고 데이터를 조직의 특정 그룹에 세그먼트화할 수 있습니다. 이렇게 하면 예를 들어 스페인의 비즈니스 사용자는 일본의 비즈니스 사용자와 관련된 데이터를 볼 수 없습니다.

>[!TIP]
>
>상위 보고서 세트에서 상속된 값은 수정할 수 없습니다.

VRS는 상위 보고서 세트에 첨부된 서버측 세그먼트 정의입니다. 따라서 SDK는 히트를 상위 보고서 세트로만 전송하여 히트를 기록하므로 VRS에 데이터 수집을 수행할 수 없습니다.
...
푸시 메시지가 올바르게 전송되도록 하려면 대상자 세그먼트를 올바르게 정의해야 합니다. 자세한 내용은 [대상: 푸시 메시지에 대한 대상 세그먼트 정의 및 구성](/help/using/in-app-messaging/t-create-push-message/c-audience-push-message.md).
...
| `analytics.lifecycleTimeout` | 아니요 | 예 | 사용자가 자신의 데이터가 일관되지 않게 표시되도록 하려면 상위 보고 세트여야 합니다. |
| `analytics.privacyDefault` | 아니요 | 예 |  |
| `analytics.batchLimit` | 아니요 | 예 |  |
| `analytics.timezone` | 예 | 예, 앱을 처음 만들 때. | 이 시간대 속성은 Adobe Analytics으로 데이터를 전송하는 데 사용되며 VRS가 생성될 때 설정된 시간대 속성과 다릅니다. |
