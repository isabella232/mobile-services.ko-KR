...
Mobile Services UI에서 VRS를 사용하여 앱을 만들고 조직의 특정 그룹에 데이터를 세그먼트화할 수 있습니다. 예를 들어 스페인의 비즈니스 사용자는 일본의 비즈니스 사용자와 관련된 데이터를 볼 수 없습니다.

>[!TIP]
>
>상위 보고서 세트에서 상속된 값은 수정할 수 없습니다.

VRS는 상위 보고서 세트에 첨부된 서버측 세그먼트 정의입니다. 따라서 SDK는 히트를 상위 보고서 세트에만 전송하여 히트를 기록하므로 VRS에 데이터 수집을 수행할 수 없습니다.
...
| `analytics.lifecycleTimeout` | 아니요 | 예 | 사용자가 데이터가 일치하지 않도록 하는 경우 상위 보고 세트여야 합니다. |
| `analytics.privacyDefault` | 아니요 | 예 |  |
| `analytics.batchLimit` | 아니요 | 예 |  |
| `analytics.timezone` | 예 | 예, 앱을 처음 만들 때 | 이 시간대 속성은 Adobe Analytics으로 데이터를 전송하는 데 사용되며 VRS가 생성될 때 설정되는 표준 시간대 속성과 다릅니다. |
